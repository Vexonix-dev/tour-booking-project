  ...
  >
    <div class="bg-zinc-900 text-white rounded-2xl p-6 sm:p-8 w-full max-w-md shadow-2xl relative">
      ...
      <form @submit.prevent="handleLogin" class="space-y-4">
        <div>
          ...
          <input
            v-model="email"
            type="email"
            class="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2 text-white focus:ring-violet-500 focus:border-violet-500"
        ...
        <button
          type="submit"
          class="w-full bg-violet-500 hover:bg-violet-600 text-white font-semibold py-2 rounded-lg transition"
        >
          Войти
        </button>
      </form>
      ...
      <p class="text-sm text-center mt-6 text-gray-400">
        Нет аккаунта?
        <router-link to="/register" class="text-violet-500 hover:underline">Зарегистрироваться</router-link>
      </p>
    </div>
  </div>
</template>